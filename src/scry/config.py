...
def get_scry_dir(project_root: Path | None = None) -> Path:
    root = project_root or get_project_root()
    return root / ".scry"


def get_db_path(project_root: Path | None = None) -> Path:
    return get_scry_dir(project_root) / "data" / "project.db"


def get_lock_path(project_root: Path | None = None) -> Path:
    return get_scry_dir(project_root) / "runtime" / "lock"


def get_scripts_dir(project_root: Path | None = None) -> Path:
    return get_scry_dir(project_root) / "scripts"


MarkerMode = Literal["inline", "sidecar", "off"]

_VALID_MARKER_MODES: frozenset[str] = frozenset({"inline", "sidecar", "off"})


def get_marker_mode(project_root: Path | None = None) -> MarkerMode:
    """Read marker_mode from config.toml. Defaults to 'inline'."""
    config_path = get_scry_dir(project_root) / "config.toml"
